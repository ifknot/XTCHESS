...
TEST(bios_video_services) {
    ...
    bios_write_character_and_attribute_at_cursor('X', 0x80, 80, video.page); // top row blinking Xs
    chattr =  bios_read_character_and_attribute_at_cursor(video.page);
        EXPECT_EQ(chattr.pair.chr, 'X');
        EXPECT_EQ(chattr.pair.attr, 0x80); 
    bios_write_character_at_cursor('0', 0, 80, video.page); // top row flashing 0s (inherit attributes)
    chattr =  bios_read_character_and_attribute_at_cursor(video.page);
        EXPECT_EQ(chattr.pair.chr, '0');
        EXPECT_EQ(chattr.pair.attr, 0x80); 
    bios_write_text_teletype_mode('!', 0, video.page); // advance cursor into row of zeros 
    chattr =  bios_read_character_and_attribute_at_cursor(video.page);
        EXPECT_EQ(chattr.pair.chr, '0');
        EXPECT_EQ(chattr.pair.attr, 0x80); 
